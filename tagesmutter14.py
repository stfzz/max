...
def check_FehlerEingewöhnung543Notbetreuung(df):
    ...
    if not df[data_inizio_minima & data_inizio_massima & ore_543].empty:
        expndr = st.expander("Trovato errore Eingewöhnung 543 Notbetreuung (data inizio >= 26.10.2020 e <= 16.11.2020 e ore 543 > 0)")
        with expndr:
...
def check_InizioMinoreFine(df):
    ...
    if not df[inizio_minore_fine].empty:
        expndr = st.expander("Trovato errore date contrattuali (Vertragsbeginn > Vertragsende)")
        with expndr:
...
def check_ErroreDati543(df):
    ...
    if not df[errore_dati_543p1 & errore_dati_543p2 & errore_dati_543p3].empty:
        expndr = st.expander("Trovato errore dati 543 (Vertragsende vor 05.03.20 UND Vertragsbeginn nach 18.05.20 dann dürfen Stunden 543 nicht 0 sein)")
        with expndr:
...
def check_FineAssistenzaMax4Anni(df):
    ...
    l = len(df[giorni])
    if not df[giorni].empty:
        expndr = st.expander("Trovato errore fine contratto assistenza (Vertragsende darf höchstens 4 Jahre größer als Geburtsdatum sein)")
        with expndr:
            gridOptions = buildGrid(df[giorni])
            AgGrid(df[giorni], gridOptions=gridOptions, enable_enterprise_modules=True)
            df.loc[
...
def check_Kindergarten_1(df):
    ...
    if not df[data_nascita & data_fine_assistenza].empty:
        expndr = st.expander("Trovato errore Kindergarten #1 (Geburtsdatum kleiner/gleich 28.02.2017 ist, dann muss Vertragende kleiner/gleich 15.09.2019)")
        with expndr:
...
def check_Kindergarten_2(df):
    ...
    if not df[data_nascita & data_fine_ass].empty:
        expndr = st.expander("Trovato errore controllo Kindergarten #2 (Geburtsdatum größer/gleich 01.03.2017 muss Vertragende <= 15.09 des Geburtsjahrs +3 Jahre sein)")
        with expndr:
...
def check_ErroreFinanziamentoCompensativo(df):
    ...
    if not df[data_inizio & ore_compensative].empty:
        expndr = st.expander("Trovato errore finanziamento compensativo (Data inizio >= 05.03.2020 e OreContrattualizzateNonErogateFase2 (finanziamento compensativo) > 0 )")
        with expndr:
...
def check_FehlerEingewöhnung(df):
    ...
    if not df[data_inizio_minima & data_inizio_massima & ore_contrattualizzate].empty:
        expndr = st.expander("Trovato errore Eingewöhnung (data inizio >= 13.02.2020 e <= 05.03.2020 e OreContrattualizzateNonErogateFase2 (finanziamento compensativo) > 0)")
        with expndr:
            gridOptions = buildGrid(
...
def check_FehlerEingewöhnung543Lockdown(df):
    ...
    if not df[data_inizio_minima & data_inizio_massima & ore_543].empty:
        expndr = st.expander("Trovato errore Eingewöhnung 543 Lockdown (Vertragsbeginn >= 13.02.2020 und <= 05.03.2020 e ore543 > 0)")
        with expndr:
            gridOptions = buildGrid(
                df[data_inizio_minima & data_inizio_massima & ore_543]
            )
...
def check_ErroreCovid(df):
    ...
    if not df[data_fine_assistenza & (ore_543 | ore_733 | ore_contrattualizzate)].empty:
        expndr = st.expander("Trovato errore Covid #1 (data inizio < 05.03.2020 e Ore543 > 0) oppure (data fine < 05.03.2020 e Ore733 > 0) oppure (data fine < 05.03.2020 e OreContrattualizzateNonErogateFase2 (finanziamento compensativo) > 0)")
        with expndr:
            gridOptions = buildGrid(
...
def check_ErroreCovid2(df):
    ...
    if not df[
        ...
        expndr = st.expander("Trovato errore Covid #2 ((data inizio >= 24.11.2020 e ore543 >0) oppure (data inizio >= 24.11.2020 e OreContrattualizzateNonErogateFase2 (finanziamento compensativo >0))")
        with expndr:
            gridOptions = buildGrid(
                df[
